...
def import_salesapp(conn, wb, filename: str | None = None) -> int:
    ...
    purpose_cols = [(h, i) for i, h in enumerate(header) if str(h).startswith("Účel")]
    ...
    def purpose(row):
        hit = [str(h).replace("Účel návštevy", "").strip(" -") for h, i in purpose_cols
    ...
    for row in it:
        ...
        batch.append((
            ...
            _g(row, hidx, "Executor"), _g(row, hidx, "Executor UID"),
            _g(row, hidx, "Date"), _g(row, hidx, "Started at"), _g(row, hidx, "Finished at"),
            _g(row, hidx, "Real duration (h)"), purpose(row), import_id))
        n += 1
        if len(batch) >= 1000:
...
def _flush_visits(conn, batch):
    conn.executemany(
        ...
        "technician, executor_uid, visit_date, started_at, finished_at, real_duration, purpose, import_id) "
