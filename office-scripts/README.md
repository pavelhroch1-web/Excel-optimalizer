...
- Each top-level file (`ImportEngine.ts`, and later `PlanningEngine.ts`,
  `AdvisorEngine.ts`, `ComplianceEngine.ts`, `ReportingEngine.ts`) is the **actual deployable
  script**: it contains a copy of only the `shared/` pieces it needs, pasted at the top of
  `main()`, followed by that engine's own logic.
